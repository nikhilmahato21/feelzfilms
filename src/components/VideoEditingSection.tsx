

const VideoEditingSection = () => {
  return (
    <section className="min-h-screen w-full bg-[url('https://images.unsplash.com/photo-1579109652910-99b9be06aaec?q=80&w=987&auto=format&fit=crop')] bg-cover bg-center flex flex-col items-center justify-center px-4 py-10 backdrop-blur-md rounded-4xl overflow-hidden">
      {/* Overlay */}
      <div className="absolute inset-0 bg-black/50 backdrop-blur-xl -z-10 " />

      <div className="text-center max-w-3xl mx-auto mb-12">
        <p className="uppercase tracking-wide text-gray-200/80 mb-2 text-sm font-medium">
          Craft
        </p>
        <h1 className="text-4xl md:text-5xl font-extrabold font-unbounded text-white mb-3 drop-shadow-lg">
          Video Editing That Moves
        </h1>
        <p className="text-gray-300 text-lg font-sans">
          Sharp cuts that capture emotion and narrative precision
        </p>
      </div>

      {/* Cards */}
      <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto w-full">
        {/* Card 1 */}
        <div className="rounded-3xl bg-white/10 border border-white/20 backdrop-blur-xl shadow-lg overflow-hidden hover:bg-white/20 transition-all duration-300">
          <img
            src="https://images.unsplash.com/photo-1490810194309-344b3661ba39?q=80&w=2896&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="Narrative Editing"
            className="w-full h-64 object-cover"
          />
          <div className="p-6 text-white">
            <p className="uppercase text-sm tracking-wide text-gray-300 mb-2">
              Style
            </p>
            <h2 className="text-2xl font-extrabold mb-3 font-unbounded">
              Narrative Techniques That Transform Ordinary Footage
            </h2>
            <p className="text-gray-300 mb-6">
              We deconstruct visual stories and rebuild them with surgical
              precision and creative vision.
            </p>
            <div className="flex gap-4">
              <button className="px-5 py-2 bg-white/20 border border-white/30 rounded-full hover:bg-white/30 transition-all text-sm">
                Explore
              </button>
              <button className="px-5 py-2 bg-white/10 border border-white/20 rounded-full hover:bg-white/20 transition-all text-sm">
                Learn →
              </button>
            </div>
          </div>
        </div>

        {/* Card 2 */}
        <div className="rounded-3xl bg-white/10 border border-white/20 backdrop-blur-xl shadow-lg overflow-hidden hover:bg-white/20 transition-all duration-300">
          <img
            src="https://images.unsplash.com/photo-1553877522-43269d4ea984?auto=format&fit=crop&w=1000&q=80"
            alt="Color Grading"
            className="w-full h-64 object-cover"
          />
          <div className="p-6 text-white">
            <p className="uppercase text-sm tracking-wide text-gray-300 mb-2">
              Technique
            </p>
            <h2 className="text-2xl font-extrabold font-unbounded mb-3">
              Color Grading That Speaks Volumes
            </h2>
            <p className="text-gray-300 mb-6">
              Subtle color shifts that elevate the emotional landscape of each
              frame.
            </p>
            <div className="flex gap-4">
              <button className="px-5 py-2 bg-white/20 border border-white/30 rounded-full hover:bg-white/30 transition-all text-sm">
                Discover
              </button>
              <button className="px-5 py-2 bg-white/10 border border-white/20 rounded-full hover:bg-white/20 transition-all text-sm">
                More →
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default VideoEditingSection;
